...
import { getTripById, confirmPaymentByAdmin } from '@/actions/tripActions';
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <Card className="shadow-xl">
            ...
            <CardSection title="Joined Participants" icon={<Users/>}>
                <div className="space-y-3">
                    {trip.participants.map(p => (
                        <div key={p.id} className="p-3 border rounded-md bg-muted/30 text-sm">
        ...
        {trip.status === 'pending_payment' && (
          <CardSection title="Payment Verification" icon={<DollarSign />}>
            {trip.transferProofImageUrl ? (
              <div className="space-y-4">
                <p className="text-sm">Payment proof has been uploaded by the user.</p>
              ...
              <p className="text-sm text-muted-foreground">No payment proof has been uploaded yet.</p>
            )}
          </CardSection>
        )}
         {trip.status === 'payment_confirmed' && trip.transferProofImageUrl && (
          <CardSection title="Payment Proof" icon={<ImageIcon />}>
                <div className="border rounded-md overflow-hidden max-w-md">
                  <NextImage src={trip.transferProofImageUrl} alt="Payment Proof" width={600} height={400} className="object-contain" data-ai-hint="payment proof"/>
                </div>
                <p className="text-sm text-green-600 mt-2">Payment has been confirmed for this trip.</p>
          </CardSection>
        )}
